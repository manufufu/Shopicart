...
// 
  settings.form.forEach( form => form.addEventListener("submit", ShopiAddCart));

  function ShopiAddCart(e) {
    e.preventDefault();
    ...
    let prdId = data.variant_id,
      ...
      if (this.readyState == 4 && this.status == 200) {
        if (theTarget !== null) {
          ...
          addToMiniCart(data, vGetRes);
        }
      }
    });
  }

  function updateMinicartItems(elems = { price, quantity, totalprice, total, getResponse },data){
